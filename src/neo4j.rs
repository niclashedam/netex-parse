...
async fn push_graph(
    ...
    while let Some(node_chunk) = node_stream.next().await {
        let mut objects = node_chunk.iter().fold("".to_owned(), |mut acc, node| {
            acc.push_str(&format!(
                "{{name: \"{}\", lat: {}, long: {} }},",
    ...
    while let Some(edge_chunk) = edge_stream.next().await {
        let mut objects = edge_chunk.iter().fold("".to_owned(), |mut acc, e| {
            let timetable = serde_json::to_string(&e.timetable).expect("failed to serialize json");
            acc.push_str(&format!(
                "{{ start: \"{}\", end: \"{}\", timetable: '{}' }},",
                graph.nodes[e.start_node].short_name, graph.nodes[e.end_node].short_name, timetable,
            ));
            acc
        });
        objects.pop();
        ...
        client.run(create).await?;
        counter += 1;
        println!("pushed {} from {} edges", counter * 255, edge_count);
    }
    Ok(())
}
